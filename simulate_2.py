...
class SimpleServer(socketserver.ThreadingMixIn, socketserver.TCPServer, Thread):
    ...
    def send_first(self, id):
        ...
        thr = self.get_sample(global_threads) # Делаем выборку из X серверов
    ...
    def get_sample(self, threads):
        thr_N = random.sample(range(len(threads)), self.x_messages) # Выбираем несколько случайных чисел
        # Проверяем, что в выборке нет отправляющего сервера (чтобы не слать самому себе)
        num = 0
        for i in thr_N:
            if self.server_address[1] == threads[i].server_address[1]:
                # Если такой элемент найден ищим элемент ему на замену (как в хэш-таблицах)
                k = 1
                j = (i + k) % len(threads)
                while (i != j):
                    if j not in thr_N and self.server_address[1] != threads[j].server_address[1]:
                        thr_N[num] = j
                        break
                    k += 1
                    j = (i + k) % len(threads)
            num += 1
        sample = []
        for i in thr_N: # Делаем выборку серверов для рассылки
            sample.append(threads[i])
        return sample
        ...
        try:
            ...
            if id:
                for i in self.server.messages_codes:
                    ...
                    thr = self.server.get_sample(global_threads)
                    # Проверяем, что в выборке нет отправляющего сервера (чтобы не слать самому себе)
                    for t in thr:
                        u =  t.server_address
